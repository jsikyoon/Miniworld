...
def process_episode(args):
    ...
    j, seed, data_dir, data_type, type = args
    dict = {}
    ...
    # ... (rest of the data generation code for a single episode)
...
#def generate_data_parallel(type, num_ep, data_dir, data_type):
#    if type == 'eval':
#        seed_offset = data_type['train']
#    else:
#        seed_offset = 0
#
#    seeds = [seed_offset + j for j in range(num_ep)]
#    args = [(j, seed, data_dir, data_type, type) for j, seed in enumerate(seeds)]
#
#    with mp.Pool(processes=3) as pool:
#        pool.map(process_episode, args)
...
def generate_data_parallel(type, num_ep, data_dir, data_type):
    ...
    with ProcessPoolExecutor(max_workers=3) as executor:
        seeds = [seed_offset + j for j in range(num_ep)]
        args = [(j, seed, data_dir, data_type, type) for j, seed in enumerate(seeds)]
        executor.map(lambda x: process_episode(*x), args)
...
for type, num_ep in data_type.items():
    generate_data_parallel(type, num_ep, data_dir, data_type)