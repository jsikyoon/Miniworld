    ...
    for j in range(num_updates):
        for step in range(args.num_steps):
            ...
            for idx, eps_done in enumerate(done):
                if eps_done:
                    episode_rewards.append(reward[idx].item())
        ...
        if j % args.log_interval == 0 and len(episode_rewards) > 1:
            ...
            print("Updates {}, num timesteps {}, FPS {} \n Last {} training episodes: mean/median reward {:.2f}/{:.2f}, min/max reward {:.2f}/{:.2f}, success rate {:.2f}\n".
                format(
                    ...
                    len(episode_rewards),
                    np.mean(list(episode_rewards)),
                    np.median(list(episode_rewards)),
                    np.min(list(episode_rewards)),
                    np.max(list(episode_rewards)),
                    np.count_nonzero(np.greater(list(episode_rewards), 0)) / len(episode_rewards)
                )
            )
